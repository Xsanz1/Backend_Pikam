...
const post_transaction = async (req, res) => {
  try {
    const { detailTransactions, transaction_typepayment } = req.body;
    ...
    if (!userUuid) { // Memastikan userUuid tersedia
    ...
    // Periksa apakah transaction_typepayment valid
    ...
    await Promise.all(detailTransactions.map(async (detailTransaction) => {
      ...
      if (detailTransaction.detailtransaction_menu) {
        ...
        const menu = await tbl_menu.findOne({ where: { menu_uuid: detailTransaction.detailtransaction_menu } });
        if (menu) {
          totalTransaction += menu.menu_price * detailTransaction.detailtransaction_qtymenu;
        }
      }
      ...
      if (detailTransaction.detailtransaction_packagemenu) {
        ...
        const packageMenu = await tbl_packagemenu.findOne({ where: { packagemenu_uuid: detailTransaction.detailtransaction_packagemenu } });
        if (packageMenu) {
          totalTransaction += packageMenu.packagemenu_price * detailTransaction.detailtransaction_qtypackagemenu;
        }
      }
    }));
    ...
    console.log('Formatted Transaction Date:', transactionDateFormatted); // Debugging log
    ...
    const create_transaction = await tbl_transaction.create({
      ...
      transaction_customer: userUuid, // Menambahkan informasi pelanggan yang login berdasarkan uuid
      transaction_typepayment: transaction_typepayment, // Menambahkan tipe pembayaran
      transaction_statustransaction: '0' // Status awal
    });

    console.log('Created Transaction:', create_transaction); // Debugging log
    ...
    res.status(200).json({
      ...
      data: {
        ...
        transaction_total: totalTransaction, // Pastikan totalTransaction disertakan dalam response
        transaction_code: transaction_code,
        transaction_customer: userUuid, // Menampilkan informasi pelanggan yang login berdasarkan uuid
        transaction_typepayment: transaction_typepayment, // Menampilkan tipe pembayaran
        transaction_statustransaction: '0' // Status awal
      },
    });
