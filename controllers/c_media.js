...
const post_upload_media = async (req, res) => {
  upload.single('file')(req, res, async (error) => {
    ...
    try {
      ...
      const { value, error: validationError } = uuidSchema.validate(req.params);
      if (validationError) {
      ...
      const [
        ...
        tbl_packagemenu.findOne({ where: { packagemenu_uuid: table_uuid } }),
      ...
      if (menuExists) {
        ...
        return res.status(404).send('UUID tidak ditemukan dalam entitas mana pun.');
      }
      ...
      if (entityType === 'menu') {
        await tbl_menu.update({ menu_image: url }, { where: { menu_uuid: table_uuid } });
      } else if (entityType === 'user') {
        await tbl_user.update({ user_media: url }, { where: { user_uuid: table_uuid } });
      } else if (entityType === 'customer') {
        await tbl_customer.update({ customer_media: url }, { where: { customer_uuid: table_uuid } });
      } else if (entityType === 'packagemenu') {
        await tbl_packagemenu.update({ packagemenu_image: url }, { where: { packagemenu_uuid: table_uuid } });
      } else if (entityType === 'typemenu') {
        await tbl_typemenu.update({ typemenu_image: url }, { where: { typemenu_uuid: table_uuid } });
      }
