...
const post_table = async (req, res) => {
  try {
    ...
    // Cek apakah meja sudah ada di database
    ...
    // Jika meja sudah ada, kirim respons error
    if (existingtable) {
      return res.status(400).json({
        success: false,
        message:
          "Meja atau nomor meja sudah aada di database",
        data: null,
      });
    }
    ...
    // Lanjutkan proses jika nomer belum ada
